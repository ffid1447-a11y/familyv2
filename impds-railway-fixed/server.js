...
function parseResults(html) {
    try {
        ...
        if (tables.length < 2) return { error: 'No data found' };
...
app.get('/search', async (req, res) => {
    try {
        ...
        if (!aadhaar) {
            return res.status(400).json({ success: false, error: 'Aadhaar required' });
        }
        
        const cleanAadhaar = aadhaar.replace(/\s/g, '');
        if (!/^\d{12}$/.test(cleanAadhaar)) {
            return res.status(400).json({ success: false, error: 'Invalid Aadhaar' });
        }
